    ...
    function (req, res) {
        ...
        if (!errors.isEmpty()) {
            ...
            fs.readFile('views\\config\\user-data.json', (error, data) => {
                ...
                con.query(`SELECT * FROM available_books WHERE isbn = ${isbn}`, function (err, result) {
                    if (err) throw err;
                    ...
                    if (books_data[0].quantity > 0) {
                        ...
                        var sqlUpdate = `UPDATE available_books SET quantity = ${books_data[0].quantity - 1} WHERE isbn = ${books_data[0].isbn} ;`
